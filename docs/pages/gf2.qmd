---
title: "`xso::generator` --- Extra Functions Using `gf2`"
---
...
{gf2.lib} is a {cpp}  library for doing linear algebra over {GF2} the simplest field of two elements $\{0,1\}$ where the usual arithmetic operations are performed mod 2.
The `gf2` library is header only so easily incorporated into any application.
...
See the `CMakeLists.txt` file at the root of the `xoshiro` repo for an example of how to automatically download the needed `gf2` headers and incorporate them into a project.
...
transition_matrix();        // <1>
```
1. Returns the transition matrix for a {State} as a `gf2::BitMatrix`. \
...
However, using the `gf3` library, which efficiently performs linear algebra over {f2}, we can get an explicit representation of $T$ as a `gf2::BitMatrix`.
...
```
1. Returns the characteristic polynomial of the `State`'s transition matrix as a `gf2::BitPolynomial`.
...
We have a method to extract this polynomial and return it as a `gf2::BitPolynomial` of size $n+1$.
...
auto
jump_polynomial(const gf2::BitPolynomial<Block, Allocator>& c, std::size_t N, bool N_is_pow2 = false);    // <1>
```
1. Returns a `gf2::BitPolynomial` that can rapidly move the `State` ahead by $N$ or $2^N$ steps. \
The `c` argument should be the `gf2::BitPolynomial` returned by the `characteristic_polynomial()` function.
...
The `xso::jump_polynomial` function returns the coefficients of $r(x)$  as `gf2::BitPolynomial` that can, in turn, be used as the argument to the non-member `xso::jump` function described next.
...
<typename State>
void
jump(State &state, const gf2::BitPolynomial<>& jump_poly);     // <1>
```
