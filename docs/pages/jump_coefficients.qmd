...
These functions returns the coefficients of a polynomial that will jump the generator or `State` ahead by either $J = N$ or $J = 2^N$ steps.
...
Calls to this method will fail if the `State` not have its  *pre-computed* characteristic coefficients embedded in the `xoshiro.h` header file.
...
For a jump of size $J$ the *jump polynomial* is the residual of $x^J$ with respect to $c(x)$:
$$
    j(x; J) \equiv \mod{x^J}{c(x)}.
$$
For a given jump size $J$ our function first gets the *precomputed* coefficients for $c(x)$ by calling to [`characteristic_coefficients`].

Assuming that succeeds, it then performs the needed polynomial reduction to compute the coefficients of $j(x; J)$:
$$
    j(x; J) = j_0 + j_1 x + \cdots + j_{n-1} x^{n-1}.
$$
The computation packs the $n$ coefficients $j_0, j_1, \ldots, j_{n-1}$ into $n_w$ words of an array it returns to the user.
...
```
1. The return value from `jump_coefficients` isn't hugely useful by itself but it can be used to jump to multiple locations in the random number stream.
