...
1.  The unsigned word type used to store the state which is also the output type of the generator.
2.  The number of words used to store the state. More words mean more state.
3.  Up to three parameters labeled $A$, $B$, and $C$ which determine how the state is advanced.
4.  Up to three further parameters labeled $R$, $S$, and $T$ which determine how the state is scrambled to produce the required single word of output.
5.  Up to two indices that determine the words of the state taking part in the output function.

The code in `xoshiro.h` covers all possible cases but typing that many parameters is a pain.
Moreover, while we can instantiate any generator in the family, only a very limited number of those have ever been properly vetted for suitability as being "good".

Therefore, we provide a few short *type aliases* for the generators you should use in most cases.
...
These generators correspond to picking specific `A`, `B`, and `C` parameters for the state advance method and specific `R`, `S` and `T` parameters for the output/scrambler function.
The choices are shown in more detail in rows 11 and 14 of the next table below.

## Analyzed Generators

Beyond the recommended and simplest type aliases above we also provide rationally named type aliases for 17 variants that are analyzed in some detail in this original [paper].
