...
Extra non-member functions for generator analysis are defined if the {gf2.lib} library is available.

`gf2` is a {cpp}  library for doing linear algebra over {GF2} the simplest Galois field of two elements $\{0,1\}$, where the usual arithmetic operations are performed mod 2.
The `gf2` library is header-only, so it can be easily incorporated into any application.
...
NOTE: Even if you only use the single `xoshiro.h` header file without incorporating the `gf2` library, you can still jump any of the predefined type-aliased *xoshiro/xoroshiro* variants by *arbitrary* numbers of steps.
