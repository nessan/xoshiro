...
```
1. Default constructor fully seeds the generator primarily from a [`std::random_device`].
...
1. The default constructor seeds the underlying state *fully* from a default source of entropy by multiple calls to [`std::random_device`].
...
To mitigate some of the worst features of this type of construction, the implementation uses a scrambled version of the one input word as a seed to a simple, small state, but pretty effective [*SplitMix64*][SplitMix] random number generator. That generator is then used to create a full state array for the *xoshiro/xoshiro* in question.
