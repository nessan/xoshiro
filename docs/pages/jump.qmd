...
If, instead, you are using some other variant of xoshiro/xoroshiro, you can still get its characteristic polynomial and jump polynomials by employing the extra, header-only, {gf2.lib} library.
That is a package for {GF2}, which can compute the needed characteristic polynomials for the generator's transition matrix.
Once the `gf2` library is included, then the `xoshiro.h` header file defines extra non-member functions that can compute those characteristic and jump polynomials.

See the documentation for these functions at {gf2.lib} website.
...
{jump_coefficients}                         \
{partition}                                 \
{gf2.characteristic_polynomial}             \
{gf2.reduce}

{jump-technique}                            \
{gf2.reduction-technique}
