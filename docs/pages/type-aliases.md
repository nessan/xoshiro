...
Our _xoshiro/xoroshiro_ implementations depend on up to 10 template parameters:
...
3.  Up to three parameters, labelled $A$, $B$, and $C$, which determine how the state is advanced.
4.  Up to three parameters, labelled $R$, $S$, and $T$, which determine how the state is scrambled to produce the required single output word.
5.  Up to two indices that determine the words of the state taking part in the scrambler.

The code in `xoshiro.h` covers all possible cases, but typing that many parameters is not ergonomic.
...
Therefore, we provide a few _type aliases_ for the generators you should use in most cases.
...
Beyond the recommended and simplest type aliases above, we also provide rationally named type aliases for 17 variants, which are analysed in some detail in the [original paper].
...
[`xso::generator`](generator.md) for information about the general implementation.
