...
It connects a [`State`] with an [`Scrambler`] to create a pseudorandom number generator.

::: {.callout-tip}
# Most users should just use one of the predefined type aliases
...
This is useful as some functions work naturally on a `State` but you can just pass the `generator` along in its place as a proxy.
:::
...
These types and methods are used by the `generator` as described next.
...
[`jump`]                        | This function uses the output of `jump_coefficients` to efficiently jump ta `generator` or `State` ahead by a potentially huge number of steps.
...
### Dependent on the `bit` Library

If the [`bit`] library is available *extra* non-member functions are defined.
...
[`jump`][performing jumps]      | Uses a jump polynomial to efficiently jump a `State` ahead in its stream.
: {.bordered .striped .hover .responsive tbl-colwidths="[40,60]"}