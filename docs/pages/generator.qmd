...
### Depending on the `gf2` Library

If the {gf2.lib} library is available, extra non-member functions will be defined.

`gf2` is a {cpp}  library for doing linear algebra over {GF2} the simplest field of two elements $\{0,1\}$, where the usual arithmetic operations are performed mod 2.
...
Function                        | Description
------------------------------- | ---------------------------------------------------
{transition_matrix}             | Returns the *transition matrix* for a `State` as a {gf2.matrix}.
{characteristic_polynomial}     | Returns the transition matrix's {characteristic-polynomial} as a {gf2.polynomial}.
