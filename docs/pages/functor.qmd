...
```
1. Reduces the state to a single output word and also calls `step()` to prepare for the next call. \
This operator is required by the [`std::uniform_random_bit_generator`] concept.
1. This is a sub-operation of the `operator()()` method that advances the state by one step without producing any output.

::: {.callout-tip}
For the most part, users will rely on `operator()()` which calls `step()` as part of its process.
:::
